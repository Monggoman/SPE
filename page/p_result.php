...
<li><a class="subheader">Student Performance Evaluator</a></li>
    <li><a href="/Student eval/page/p_home.php" class="waves-effect" disabled>Home</a></li>
		<li><div class="divider"></div></li>
		<li><a href="/Student eval/page/p_account.php" class="waves-effect">Account</a></li>
    <li><div class="divider"></div></li>
		<li><a href="#" class="waves-effect">Result </a></li>
    <li><div class="divider"></div></li>
					...
					<h6> Quiz List </h6>
						<table  class="responsive-table" id="student-table">
							...
							<tbody>
								...
								$par = $_SESSION["user_id"];
								$sql = "SELECT f_stud_id FROM parent WHERE fkuser_id_par = '$par'";
								...
								$sql = "SELECT assessment.f_sub, assessment.ass_name, user.user_fname, user.user_mname, user_lname, score, mistakes, assessment.Total_q  FROM history LEFT JOIN assessment ON history.f_ass_id = assessment.ass_id LEFT JOIN user ON history.f_user_id = user.user_id WHERE f_user_id = '$stud_id'";
								$result = $conn->query($sql);
								
								if ($result->num_rows > 0) {
									while ($row = $result->fetch_assoc()) {
										echo "<tr row_id='" . $row["f_sub"] . "'><td>" . $row["f_sub"] . "</td><td>" . $row["ass_name"] . "</td></td>" . "<td>" . $row["user_fname"] . "</td></td>". "<td>" . $row["user_mname"] . "</td></td>". "<td>" . $row["user_lname"] . "</td></td>". "<td>" . $row["score"] . "</td></td>" . "<td>" . $row["mistakes"] . "</td></td>". "<td>" . $row["Total_q"] . "</td></td>";
									}
					...
					<h6> Activity List </h6>
						<table  class="responsive-table" id="student-table">
							...
							<tbody>
								...
								$sql = "SELECT activity.act_id, activity.fr_sub, activity.act_title, user.user_fname, user.user_mname, user_lname, score, status1 FROM act_history LEFT JOIN activity ON act_history.f_act_id = activity.act_id LEFT JOIN user ON act_history.fuser_id = user.user_id WHERE fuser_id = '$stud_id'";
								$result = $conn->query($sql);
