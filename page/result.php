					...
					<h6> Quiz List </h6>
						<table  class="responsive-table" id="student-table">
							...
							<tbody>
								...
								$sql = "SELECT assessment.f_sub, assessment.ass_name, score, mistakes, assessment.Total_q  FROM history LEFT JOIN assessment ON history.f_ass_id = assessment.ass_id WHERE f_user_id = '$_SESSION[user_id]'";
								$result = $conn->query($sql);
								
								if ($result->num_rows > 0) {
									while ($row = $result->fetch_assoc()) {
										echo "<tr row_id='" . $row["f_sub"] . "'><td>" . $row["f_sub"] . "</td><td>" . $row["ass_name"] . "</td></td>" . "<td>" . $row["score"] . "</td></td>" . "<td>" . $row["mistakes"] . "</td></td>". "<td>" . $row["Total_q"] . "</td></td>";
									}
					...
					<h6> Activity List </h6>
						<table  class="responsive-table" id="student-table1">
							...
							<tbody>
								...
								$sql = "SELECT activity.fr_sub, activity.act_title, score, status1 FROM act_history LEFT JOIN activity ON act_history.f_act_id = activity.act_id WHERE fuser_id = '$_SESSION[user_id]'";
								$result = $conn->query($sql);
								
								if ($result->num_rows > 0) {
									while ($row1 = $result->fetch_assoc()) {
										echo "<tr row_id='" . $row1["fr_sub"] . "'><td>" . $row1["fr_sub"] . "</td><td>"  . $row1["act_title"] . "</td></td>"."<td>" . $row1["score"] . "</td></td>" . "<td>" . $row1["status1"] . "</td></td>";
									}
