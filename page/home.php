...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if (isset($_POST["show"])) {
		$query = "SELECT ass_name, f_sub, history.score ,Total_q FROM assessment LEFT JOIN history ON history.f_ass_id = assessment.ass_id WHERE f_sub = '$subject' AND history.f_user_id = '$user_id' ";
	...
	if (isset($_POST["show_act"])) {
		$query = "SELECT act_title, fr_sub, act_history.score FROM activity LEFT JOIN act_history ON act_history.f_act_id = activity.act_id WHERE fr_sub = '$subject' AND act_history.fuser_id = '$user_id' ";
...
<ul id="side-menu" class="sidenav side-menu">
	...
	<div class="card-panel recipe1 white row">
		...
		<br>
		<div></div>
		<button  id="update" name="show" class="btn orange" > Show Quiz Graph</button>
		<br>
		<div></div>
		<button  id="update" name="show_act" class="btn orange" > Show Activity Graph</button>
        ...
        data:{
            ...
            datasets:[{
                ...
                hoverBorderColor: 'black'
								...
								label: 'Total',
								data: <?php echo json_encode($total_q) ?>,
								type: 'bar',
