...
if (isset($_SESSION['user_id'])) {
  ...
  if ($result->num_rows > 0) {
	...
	$sql = "SELECT * FROM parent WHERE fkuser_id_par = $id";
  $result = $conn->query($sql);
  if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
			$user_fname['Contact'] = $row['Contact_no'];
			$user_fname['stud_code'] = $row['f_stud_id'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (isset($_POST["update"])) {
		...
		$sql = "UPDATE parent SET Contact_no = '$s_Contact', f_stud_id =  '$stud_code' WHERE fkuser_id_par = '$id'";
		if ($conn->query($sql) == TRUE) {
      echo '<script> alert("Data successfully Updated.") </script>';
      echo '<script> window.location.href="/Student eval/page/p_account.php" </script>';
    } else {
      ...
      <span class="right grey-text text-darken-1">
        <i class="material-icons sidenav-trigger" data-target="side-menu">menu</i>
			...
			<ul id="side-menu" class="sidenav side-menu">
				...
				<li><a href="/Student eval/page/p_home.php" class="waves-effect">Home</a></li>
			...
			<form action="p_account.php" method="post">
				<div class="recipes container grey-text text-darken-1">
					<div class="card-panel1 recipe white row">
							...
							<div></div>
							<div class="input-field">
								<input type="number" id="stud_id" name="stud_code" value ="<?php echo $user_fname['stud_code']; ?>">
								<label for="contact_no">Input your Child Student Code:</label>
							</div>
